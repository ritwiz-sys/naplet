...
// ── Desktop nav button with slide-in tooltip ──────────────────────
...
function MobileNavBtn({ item, isActive, onClick }) {
    return (
        <button
            onClick={() => onClick(item.id)}
        ...
        >
            <div className="relative flex items-center justify-center w-8 h-6 rounded-xl transition-all duration-200"
                style={isActive ? {
            ...
            <span className="text-[8px] font-semibold tracking-wide truncate w-full text-center px-0.5" style={{ fontFamily: 'var(--font-ui)' }}>
                {item.label}
            </span>
        </button>
    );
...
function Sidebar({ activePage = 'home', onNav }) {
    ...
    const settingsNav = NAV.find(n => n.id === 'settings');

    return (
        <>
            {/* ── Desktop vertical sidebar (hidden on mobile) ── */}
            <aside className="hidden md:flex relative z-10 flex-col items-center py-4 flex-shrink-0"
                style={{
                    width: 56,
